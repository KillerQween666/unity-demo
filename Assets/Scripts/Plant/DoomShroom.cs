...
public class DoomShroom : Plant {
    ...
    IEnumerator Boom() {
        AudioManager.Instance.PlayClip(Config.doomShroomBoom); // 播放爆炸音效
        // 播放樱桃炸弹爆炸特效
        ObjectPoolManager.Instance.PlayDoomShroomBoomParticalIEnumrator(transform);
        UIManager.Instance.screenFlashUI.PlayFlash(1);
